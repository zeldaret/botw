...
class ParticleConfig : sead::hostio::Node, agl::utl::IParameterIO {

public:
    ParticleConfig();
    const char* getLightMapName(unsigned int);
    void initialize(sead::Heap*, sead::Heap*);
    void applyResource(u8*);

private:
    agl::utl::IParameterObj mData;
    short unk = 0;
    sead::FixedSafeString<0x40> someString;
