...
void GolemBgmInfo::init(sead::Heap* heap) {
    BossBgmInfo::init(heap);
}

GiantBgmInfo::GiantBgmInfo() = default;

void GiantBgmInfo::init(sead::Heap* heap) {
    BossBgmInfo::init(heap);
}
...
void SandwormBgmInfo::init(sead::Heap* heap) {
    BossBgmInfo::init(heap);  // also just a pointer to BossBgmInfo function in its vtable
}
...
void DungeonNormalBgmInfo::init(sead::Heap* heap) {
    addList(&mHeader, "MusicParam");
    // TODO sizeof() or some other trick *not* to hardcode "3"?
    mPartChangeOdds.allocBufferAssert(3, heap);
    for (int i = 0; i < 3; i++)
...
mOddsPlayableString(50,"OddsPlayableString","ストリングスが選ばれる確率（％）","Min=0.f, Max=100.f",&mData),
mTitleDelaySec(0,"TitleDelaySec","ダンジョンのタイトルが消えてから再生開始までの時間(秒)","Min=0.f,Max=10.f",&mData) {}
