...
void LinkTag::onEnterCalc() {
    ...
    if (mMapObj) {
        if (mMapObj->getRevivalGameDataFlagHash() != gdt::InvalidHandle)
            mFlags.set(LinkTagFlag::_4000);
        if (mState != State::Delete && mStateFlags.isOff(StateFlags::RequestDelete)) {
            mJobFlags.setBitOn(BaseProcMgr::instance()->getCurrentExtraJobArrayIdx());
...
bool LinkTag::hasJobType(int jobType) {
    if (jobType != 3)
        return false;
...
void LinkTag::queueExtraJobPush(int jobType, bool jobFlagBit){
    if(jobType == 3){
        mJobFlags.setBitOff(1^jobFlagBit);
        if(mState != State::Delete && mStateFlags.isOff(StateFlags::RequestDelete)){
            s8 extraJobArrayId = BaseProcMgr::instance()->getCurrentExtraJobArrayIdx();
            mJobFlags.setBitOn(extraJobArrayId);
            if(!mJobFlags.isBitOn(extraJobArrayId)){
                BaseProcMgr::instance()->queueExtraJobPush(&mJob.getLink());
            }
        }
    }
