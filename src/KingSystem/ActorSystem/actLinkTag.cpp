...
bool LinkTag::isTriggered(map::ObjectLink* link, u8 linkIdx) {
    ...
    if (baseProc && (u8(baseProc->getState()) | 2) != 3) {  // some weirdness with " | 2"
        ...
        if (!linkTag) {
            ...
            if (linkTag->mFlags.isOn(LinkTagFlag::_10) ==
                linkTag->mFlags.isOn(LinkTagFlag::_4))  // probably checking for two flags?
                resetBitAfterCheck(&mTriggeredLinkFlags, linkIdx);
            setBitAfterCheck(&mTriggeredLinkFlags, linkIdx);
        } else if (link->type != map::MapLinkDefType::BasicSigOnOnly) {
            if (linkTag->mFlags.isOn(LinkTagFlag::_8) ==
                linkTag->mFlags.isOn(LinkTagFlag::_4))  // probably checking for two flags?
                resetBitAfterCheck(&mTriggeredLinkFlags, linkIdx);
            setBitAfterCheck(&mTriggeredLinkFlags, linkIdx);
        } else if (linkTag->mFlags.isOn(LinkTagFlag::_400)) {
            setBitAfterCheck(&mTriggeredLinkFlags, linkIdx);
        } else {
            resetBitAfterCheck(&mTriggeredLinkFlags, linkIdx);
        }
