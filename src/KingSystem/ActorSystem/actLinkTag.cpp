...
void LinkTag::prePushJob2() {
    if (MCMgr::instance()) {
        if (_1dd == MCMgr::instance()->calcJobCounterMaybe) {
...
bool LinkTag::isTriggered(map::ObjectLink* link, u8 linkIdx) {
    ...
    if (actor && (u8(actor->getState()) | 2) != 3) {  // some weirdness with " | 2"
        if (!actor->checkDerivedRuntimeTypeInfo(LinkTag::getRuntimeTypeInfoStatic())) {
            if (!actor->checkDerivedRuntimeTypeInfo(Actor::getRuntimeTypeInfoStatic())) {
