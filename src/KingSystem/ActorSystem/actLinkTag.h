...
public:
    ...
    bool isTriggered(map::ObjectLink* link, u8 linkIdx);
    PreDeletePrepareResult prepareForPreDelete_() override;
    bool isDonePreparingForPreDelete();
    bool hasJobType(JobType jobType);
    bool shouldSkipJobPush();
    bool isSpecialJobType();
    void queueExtraJobPush(JobType jobType, u32 jobFlagBit);
    bool canWakeUp();
    void prePushJob2();

    s8 getSelfLinkCount();

    // this doesn't even belong in this namespace, but is used as a placeholder for init() and
    // prePushJob2()
    static bool isFlagSet(bool* isSet, int unk, map::Object* object);
    // this also doesn't belong here, used in calcPulse(), calcOther() (7100d15b04);
    static void triggerLink(ActorLinkConstDataAccess& acc);
    // this also doesn't belong here, used in dtor
    static u32 getSceneStatus();

    BaseProcJobHandlerT<LinkTag> mJob;
    sead::LongBitFlag<96> mTriggeredLinkFlags;
    sead::SizedEnum<Type, s8> mLinkTagType = Type::And;
    sead::Atomic<s8> _1dd = 0xFF;
