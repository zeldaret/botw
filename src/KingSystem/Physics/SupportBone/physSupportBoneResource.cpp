...
SupportBoneResource::SupportBoneResource()
    : IParameterIO("physsb", 0), mBoneNum(0, "bone_num", "bone_num", &mSupportBoneHeader),
      mConnectionLinearNum(0, "connection_linear_num", "connection_linear_num", &mSupportBoneHeader),
      ...
      mMainBoneNum(0, "main_bone_num", "main_bone__num", &mSupportBoneHeader),
      mSupportBoneNum(0, "support_bone_num", "support_bone__num", &mSupportBoneHeader) {
...
bool SupportBoneResource::init(agl::utl::ResParameterArchive archive, sead::Heap* heap) {
    ...
    if (output_double_num > 0) {
        ...
        for (int i = 0; i < output_double_num; i++) {
            ...
            mOutputDoubleBuffer[i].weight_0.init(0, "weight_0", "weight_0", &mOutputDoubleBuffer[i]);
            mOutputDoubleBuffer[i].connection_1.init(0, "connection_1", "connection_1",
                                                    &mOutputDoubleBuffer[i]);
            mOutputDoubleBuffer[i].weight_1.init(0, "weight_1", "weight_1", &mOutputDoubleBuffer[i]);
        }
    }
...
float SupportBoneResource::getInterpolatedConnectionCurve(u32 index, float lookup) {
    ...
    if (x0 == lookup) {
        ...
        result =
            cubicHermiteInterpolate(connection_curve.key0.ref(), connection_curve.key1.ref(), lookup);
    } else if (x2 > lookup) {
        result =
            cubicHermiteInterpolate(connection_curve.key1.ref(), connection_curve.key2.ref(), lookup);
    } else {
